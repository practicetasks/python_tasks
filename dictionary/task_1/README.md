...
def add_music_band():
    ...
    # Если группы нет, запросите у пользователя песни (названия через запятую и пробел)
    # Добавьте музыкальную группу и её песни в music_library
    # Выведите сообщение об успешном добавлении или о том, что группа уже существует

def add_songs_to_band():
    # Запросите у пользователя название музыкальной группы
    # Проверьте, существует ли такая группа в music_library
    # Если группа существует, запросите у пользователя дополнительные песни (названия через запятую и пробел)
    # Добавьте песни к существующим в music_library
